...
// Вычисление суммы.
func CalculateAgeSum(collection []IHuman) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IHuman) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IHuman, instance IHuman) bool {
	for _, v := range *collection {
