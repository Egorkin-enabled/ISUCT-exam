...
// Вычисление суммы.
func CalculateAgeSum(collection []IKeyboard) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IKeyboard) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IKeyboard, instance IKeyboard) bool {
	for _, v := range *collection {
		// Сравниваем значения попарно,
		// т.к.  интерфейс - указатель и оператор == сравнивает адреса
		// Проверку size упкскаем, т.к. size зависит от проверяемых значений
