...
// Вычисление суммы.
func CalculateAgeSum(collection []IMineral) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IMineral) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IMineral, instance IMineral) bool {
	for _, v := range *collection {
		// Сравниваем значения попарно,
		// т.к.  интерфейс - указатель и оператор == сравнивает адреса
		// Проверку king упкскаем, т.к. kind зависит от проверяемых значений
