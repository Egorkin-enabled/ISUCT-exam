...
// Вычисление суммы.
func CalculateAgeSum(collection []IPencil) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IPencil) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IPencil, instance IPencil) bool {
	for _, v := range *collection {
		// Сравниваем значения попарно,
		// т.к.  интерфейс - указатель и оператор == сравнивает адреса
		// Проверку color упкскаем, т.к. color зависит от проверяемых значений
