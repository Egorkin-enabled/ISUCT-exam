...
// Вычисление суммы.
func CalculateAgeSum(collection []IYacht) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IYacht) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IYacht, instance IYacht) bool {
	for _, v := range *collection {
