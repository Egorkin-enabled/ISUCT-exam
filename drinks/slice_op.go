...
// Вычисление суммы.
func CalculateAgeSum(collection []IDrink) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IDrink) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IDrink, instance IDrink) bool {
	for _, v := range *collection {
