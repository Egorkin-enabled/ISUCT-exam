...
// Вычисление суммы.
func CalculateAgeSum(collection []IProgram) uint32 {
	var result uint32 = 0
...
// Вычисление среднего.
func CalculateAgeAvg(collection []IProgram) (float64, error) {
	if len(collection) == 0 {
...
func TryAddUniqueInstance(collection *[]IProgram, instance IProgram) bool {
	for _, v := range *collection {
